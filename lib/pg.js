...
function buildSslConfig(urlHost, sslServername) {
    if (isLocalHost(urlHost) || process.env.PGSSLMODE === 'disable') {
        return false;
    }
    const ssl = { rejectUnauthorized: false };
    const sn = (process.env.PG_SSL_SERVERNAME || sslServername || '').trim();
    // SNI فقط عند اسم مضيف كامل (شهادة Render). المضيف الداخلي dpg-xxx-a بدون نقطة.
    if (sn.includes('.')) {
        ssl.servername = sn;
    }
    return ssl;
...
/** أخطاء اتصال مؤقتة — إعادة المحاولة بعد إعادة إنشاء الـ pool */
...
function buildPoolConfig(databaseUrl) {
    ...
    let ssl = buildSslConfig(host, sslServername);
...
/** استعلام مع إعادة محاولة عند انقطاع مؤقت (للإنتاج) */
async function runQuery(text, params) {
    const retries = Math.max(0, Math.min(Number(process.env.PG_QUERY_RETRIES || 2), 5));
    let lastErr;
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            lastErr = e;
            if (isTransientPgError(e) && attempt < retries) {
                console.warn(`PostgreSQL query retry ${attempt + 1}/${retries}:`, e.message || e);
                await resetPool();
...
/** نبض دوري يمنع إغلاق الاتصالات الخاملة (اختياري — مفعّل على Render) */
let keepAliveTimer = null;

function startPgKeepAlive() {
    if (keepAliveTimer || !getDatabaseUrl()) return;
    if (process.env.PG_KEEPALIVE !== '1' && !process.env.RENDER) return;
    const intervalMs = Math.max(60000, Number(process.env.PG_KEEPALIVE_MS || 300000));
    keepAliveTimer = setInterval(() => {
        runQuery('SELECT 1').catch((e) => {
            console.warn('PostgreSQL keepalive failed:', e.message || e);
        });
    }, intervalMs);
