...
/**
 * pg v8 + pg-connection-string: وضع sslmode=require في الرابط يُعامل حالياً كـ verify-full
 * فيفشل التحقق من الشهادة مع Render. الإضافة الموصى بها: uselibpqcompat=true
 * (انظر تحذير Node عند الاتصال).
 */
function normalizeDatabaseUrl(databaseUrl) {
    const raw = String(databaseUrl || '').trim();
    if (!raw || process.env.PGSSLMODE === 'disable') {
        return raw;
    }
    ...
    if (/uselibpqcompat=true/i.test(raw)) {
        return raw;
    }
    const joiner = raw.includes('?') ? '&' : '?';
    return `${raw}${joiner}uselibpqcompat=true`;
}
...
function getPool() {
    if (!pool) {
        ...
        const useIpv4Lookup = process.env.PG_FORCE_IPV4 === '1';
        const poolOpts = {
            connectionString,
