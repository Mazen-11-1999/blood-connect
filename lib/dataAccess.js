...
const { getDatabaseUrl } = require('./pg');
...
async function initDataLayer() {
    if (usePg()) {
        ...
        const maxAttempts = isProd
            ? Math.max(1, Math.min(Number(process.env.PG_INIT_RETRIES || 8), 30))
            : Math.max(1, Math.min(Number(process.env.PG_INIT_RETRIES || 2), 5));
        const delayMs = isProd
            ? Math.max(500, Math.min(Number(process.env.PG_INIT_RETRY_MS || 2500), 60000))
