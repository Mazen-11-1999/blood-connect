...
async function initDataLayer() {
    if (usePg()) {
        ...
        const maxAttempts = Math.max(1, Math.min(Number(process.env.PG_INIT_RETRIES || 8), 30));
        const delayMs = Math.max(500, Math.min(Number(process.env.PG_INIT_RETRY_MS || 2500), 60000));
        let lastErr;
