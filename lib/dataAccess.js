...
const { getDatabaseUrl, runQuery } = require('./pg');

const usePg = () => !!getDatabaseUrl();
...
async function initDataLayer() {
    if (usePg()) {
        const { initSchema, resetPool } = require('./pg');
        const isProd = process.env.NODE_ENV === 'production' || !!process.env.RENDER;
        const maxAttempts = isProd
            ? Math.max(1, Math.min(Number(process.env.PG_INIT_RETRIES || 15), 30))
            : Math.max(1, Math.min(Number(process.env.PG_INIT_RETRIES || 2), 5));
        const delayMs = isProd
            ? Math.max(1000, Math.min(Number(process.env.PG_INIT_RETRY_MS || 3000), 60000))
            : Math.max(300, Math.min(Number(process.env.PG_INIT_RETRY_MS || 800), 5000));
        let lastErr;
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                lastErr = e;
                console.error(`تهيئة PostgreSQL — المحاولة ${attempt}/${maxAttempts}:`, e.message || e);
                await resetPool();
...
async function findAllUsers() {
    ...
    const { rows } = await runQuery(`${PG_USER_SELECT} ORDER BY created_at DESC`);
...
async function findUserById(id) {
    ...
    const { rows } = await runQuery(`${PG_USER_SELECT} WHERE id = $1`, [id]);
...
async function findUserByEmail(email) {
    ...
    const { rows } = await runQuery(`${PG_USER_SELECT} WHERE email = $1`, [email]);
    return rows[0] ? rowToUser(rows[0]) : null;
}
...
async function findAllMessages() {
    ...
    const { rows } = await runQuery('SELECT * FROM messages ORDER BY created_at DESC');
...
async function findMessageById(id) {
    ...
    const { rows } = await runQuery('SELECT * FROM messages WHERE id = $1', [id]);
...
async function updateMessage(id, partial) {
    ...
    vals.push(id);
    await runQuery(`UPDATE messages SET ${sets.join(', ')} WHERE id = $${vals.length}`, vals);
...
async function deleteMessage(id) {
    ...
    const r = await runQuery('DELETE FROM messages WHERE id = $1', [id]);
    return r.rowCount > 0;
}
...
async function deletePushSubscriptionByEndpoint(endpoint) {
    ...
    await runQuery('DELETE FROM push_subscriptions WHERE endpoint = $1', [endpoint]);
}
